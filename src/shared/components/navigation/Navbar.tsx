      ...
      <nav className="flex items-center justify-between px-6 md:px-12 py-3 bg-transparent">
        ...
        <div className="relative">
          {!isAuthenticated || !user ? (
            <Link href="/login">
              <button className="px-5 py-2 rounded-2xl border border-white/30 text-white backdrop-blur-md hover:bg-white/10 transition-all duration-300 text-sm">Iniciar sesión</button>
            </Link>
          ) : (
