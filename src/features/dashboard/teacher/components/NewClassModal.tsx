...
import { NewClassModalProps, NewClassData } from "../types";
...
const DEFAULT_FORM: NewClassData = {
  emoji: "📚",
  name: "",
  studentCount: 0,
  isActive: true,
...
const NewClassModal = ({ isOpen, onClose, onSubmit }: NewClassModalProps) => {
  const [form, setForm] = useState<NewClassData>(DEFAULT_FORM);
  const [errors, setErrors] = useState<
    Partial<Record<keyof NewClassData, string>>
  >({});
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-700">Nueva Clase</h2>
          <button
            type="button"
            onClick={handleClose}
        ...
        <div>
          ...
          />
          {errors.studentCount && (
            <p className="text-xs text-red-500 mt-1">{errors.studentCount}</p>
          )}
        </div>
