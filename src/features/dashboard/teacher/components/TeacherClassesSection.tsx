...
import type { TeacherClassProgress } from '../services';
...
const TeacherClassesSection = ({ classes, selectedClassId, classProgress, onSelectClass }: TeacherClassesSectionProps) => {
  return (
    <article className="cosmic-panel border-0 shadow-[0_18px_40px_rgba(2,6,26,0.58)]">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-white/70">{classes.length} clases</span>
      </div>
          ...
          {classes.map((item) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl px-3 py-3 text-left transition ${isSelected ? 'bg-white/18' : 'bg-white/10 hover:bg-white/15'}`}
              >
                <div className="mb-2 flex items-center justify-between gap-2">
                  <p className="text-sm font-bold text-white">{item.name}</p>
                  <span className="rounded-full bg-emerald-500/25 px-2 py-1 text-[10px] font-bold text-emerald-200">Activa</span>
                </div>
