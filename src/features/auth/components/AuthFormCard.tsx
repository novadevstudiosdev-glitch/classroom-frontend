...
const getNameParts = (name: string) => {
  ...
  if (parts.length <= 1) {
    return { firstName: parts[0] || "Usuario", lastName: "Novi" };
  }
    ...
    try {
      if (isRegister) {
        if (registerRole === "parent" && !acceptTerms) {
          throw new Error("Confirma que sos padre, madre o tutor legal.");
        }
        ...
        setSuccessMessage(
          registerRole === "teacher"
            ? "Cuenta docente creada. Revisa tu email para verificarla y luego inicia sesión."
            : registerRole === "parent"
              ? "Cuenta familiar creada. Revisa tu email para verificarla y luego inicia sesión."
              : "Cuenta de alumno creada. Ya podes iniciar sesión."
        );
      ...
      setErrorMessage(extractApiErrorMessage(error, "No se pudo completar la operación. Intentalo de nuevo."));
    } finally {
    ...
    >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {isRegister ? (
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                  ...
                  <p className="text-xs text-slate-600 mt-1">
                    {registerRole === "teacher"
                      ? "Completa los datos para crear tu cuenta docente."
                      : registerRole === "student"
                        ? "Completa tus datos para crear tu cuenta de alumno."
                        : "Completa los datos del adulto responsable para crear la cuenta."}
                  </p>
              ...
              {isRegister && registerRole === "student" ? (
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">Alias del alumno</label>
                  <div className="relative">
                    <User className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                    <input
                      type="text"
                      value={formData.studentAlias}
                      onChange={(event) => setFormData({ ...formData, studentAlias: event.target.value })}
                      className="w-full pl-10 pr-4 py-3.5 border-2 border-slate-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all text-slate-800 font-medium"
              ...
              {isRegister && registerRole === "student" ? (
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">Codigo de clase (opcional)</label>
                  <div className="relative">
                    <User className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                    <input
                      type="text"
              ...
              {isRegister ? (
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    ...
                    />
                    <span className="text-sm text-slate-600 group-hover:text-slate-800 font-medium">Recordarme</span>
                  </label>
                  <button
                    type="button"
              ...
              <p className="text-center text-sm text-slate-600 mt-4">
                {isRegister ? "Ya tenes cuenta? " : "No tenes una cuenta? "}
                <button
                  type="button"
                  onClick={() => setMode((currentMode) => (currentMode === "login" ? "register" : "login"))}
                  className="font-bold text-blue-600 hover:text-blue-700"
                >
                  {isRegister ? "Inicia sesión" : "Registrate aqui"}
                </button>
              </p>
            </form>
          </motion.div>
      ...
      >
        <p className="text-xs text-slate-300 italic">
          &quot;El aprendizaje es un tesoro que seguira a su dueño en todas partes&quot;
        </p>
      </motion.div>
    </motion.div>
  );
